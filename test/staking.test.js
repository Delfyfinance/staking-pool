...
describe("Staking ", ()=>{
  // it ("add Pool", async()=>{
  //   await stakingPool.addPool(delfyToken.address, toBigNumber("40"), false, overrides)
  //    expect(await stakingPool.poolLength()).to.equal(toBigNumber("1"))
  //    expect((await stakingPool.getPool(delfyToken.address)).lpToken).to.equal(delfyToken.address)
  // })
  ...
  //   let multiplier = toBNumber(10)
  //   let poolInfo = await stakingPool.getPool(delfyToken.address)
  //   let poolSupply = toBNumber(poolInfo.poolSupply.toString())
  //   let allocPoint = toBNumber(poolInfo.allocPoint.toString())
  //   let delfyPerBlock = expandToEthers("40")
  //   let totalAllocPoint = toBNumber((await stakingPool.totalAllocPoint()).toString())
  //   let accruedShare = toBNumber(poolInfo.accDelfyPerShare.toString())
  //   let user = await stakingPool.getUserPoolInfo(delfyToken.address, other0.address)
  //   let rwdDebt = toBNumber(user.rewardDebt.toString())
  //   let pendingRwd = toBNumber(user.pendingReward.toString())
  //   let userAmt = toBNumber(user.amount.toString())
  ...
  // })
    // /* Review the contract for this test */
  // it("allow claim", async()=>{
  //    await delfyToken.connect(other0).approve(stakingPool.address, expandTo18Decimals("1000000"), overrides)
  //   await expect( stakingPool.connect(other0).deposit(delfyToken.address, expandTo18Decimals("50000"), false, overrides)).to.emit(stakingPool, "Deposit").withArgs(other0.address, delfyToken.address, expandTo18Decimals("50000"))
  //   expect(await delfyToken.balanceOf(stakingPool.address)).to.equal(expandTo18Decimals("50000"))
  //    await advanceBlock(15)
  //   const claimable = await stakingPool.pendingDelfy(delfyToken.address, other0.address)
  //   console.log(claimable.toString());
  //   await expect ( stakingPool.connect(other0).claim(delfyToken.address,overrides)).to.emit(stakingPool, "Claimed").withArgs(other0.address, delfyToken.address, claimable)
  // })
